 ...
 * sync word appears, then reads the BCD fields. Frame rate locks from
 * second-rollover consensus (bit clock until then); the drop-frame
...
/* The LTC engine (decoder, encoder, channel extractor) lives with the
   Timers module and is shared from there — one implementation, used both
   by the module's built-in listener and by this standalone reader. */
const { LtcDecoder, LtcEncoder, ChannelExtractor, frameBits, fmtTc } =
  require('../modules/propresenter-timers/ltc.js');
